...
public class ProductMgrApi {
    ...
    public ResponseResult getNo(@RequestParam Map<String, Object> params) {
        ...
        try {
            ...
            if (idRes.getRetCode().equals(ResponseResult.CODE_SUCCESS)) {
                ...
                idMap.put("seqId", IdUtil.randomUUID());
                //商品编号=YYYYMMDD+业务类型+ID版本+ID
                idMap.put("productCode",
                        DateUtil.format(DateUtil.date(), DatePattern.PURE_DATE_FORMAT) + "" + IdGeneratorService.BIZ_TYPE_PRODUCT + "" + retMap.get("version") + "" + retMap.get("currentId")
                );
                res.setData(idMap);
            } else {
    ...
    public ResponseResult get(@RequestParam Map<String, Object> params) {
        ...
        try {
            ...
            params.put("productSeqId", String.valueOf(params.get("seqId")));
